...
async def webuploader(chat_id: int, msg_id: int, uploader: str):
    ...
    sites = {
        ...
        "litterbox": {"url": "https://litterbox.catbox.moe/resources/internals/api.php", "json": False},
    ...
    if uploader and uploader in sites:
        ...
        return "```⚠️ Uploader not supported or invalid.```"
    ...
    try:
        if uploader == "filebin":
            ...
            if process.returncode == 0:
                ...
                return f"```⚠️Failed to upload file to Filebin: {stderr.strip()}```"
            ...
            try:
                ...
                return f"```⚠️ Failed to upload file: {e}```"
            ...
            raise ValueError("```⚠️Uploader not supported```")
        ...
        if process.returncode == 0:
            ...
            return f"```⚠️ Failed to upload file: {stderr.strip()}```"
    except Exception as e:
        return f"```⚠️Failed to upload file: {e}```"
    ...
    async def get_response_gemini_oracle(
        ...
        async def evaluate_response(response):
            if response.status != 200:
                return f"```⚠️Error status: {response.status}```"
        ...
        try:
            ...
            return f"```⚠️ Error connecting to the server: {client_err}```"
