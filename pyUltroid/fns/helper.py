...
def ErrInfo(exception, full=False):
    ...
    if full:
        ...
        LOGS.info(
            f"<blockquote>{exception_message}\n{detailed_info_str}\nFile: {filename}, Line {exception_traceback.tb_lineno}</blockquote>"
        )
    else:
        LOGS.info(
            f"<blockquote>{exception_message} {exception_type} {filename}, Line {exception_traceback.tb_lineno}</blockquote>"
        )
...
def inline_mention(user, custom=None, html=False):
    ...
    if isinstance(user, types.User):
        ...
        return f"<blockquote>[{mention_text}](tg://user?id={user.id})</blockquote>"
...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if not event.reply_to:
            return await eod(
                event, (f"<blockquote>Please use `{HNDLR}install` as reply to a .py file.</blockquote>", parse_mode="html")
            )
        ...
        if plug in list(LOADED):
            return await eod(ok, f"<blockquote>Plugin `{plug}` is already installed.</blockquote>", parse_mode="html")
        ...
        plug = sm.replace(".py", "")
        if plug in HELP:
            output = "<blockquote><b>Plugin</b> - `{}`\n</blockquote>".format(plug)
            for i in HELP[plug]:
                output += i
            output += "\n<blockquote>© @RemainsAlways</blockquote>"
            await eod(ok, f"✓ `Ultroid - Installed`: `{plug}` ✓\n\n{output}", parse_mode="html")
        elif plug in CMD_HELP:
            output = f"<blockquote>Plugin Name-{plug}\n\n✘ Commands Available-\n\n</blockquote>"
            output += str(CMD_HELP[plug])
            await eod(ok, f"<blockquote>✓ `Ultroid - Installed`: `{plug}` </blockquote>\n\n{output}", parse_mode="html")
        else:
            try:
                x = f"<blockquote>Plugin Name-{plug}\n\n✘ Commands Available-\n\n</blockquote>"
                for d in LIST[plug]:
                    x += HNDLR + d + "\n"
                await eod(ok, f"<blockquote>✓ `Ultroid - Installed`: `{plug}` ✓\n\n`{x}`</blockquote>", parse_mode="html")
            except BaseException:
                await eod(ok, f"<blockquote>✓ `Ultroid - Installed`: `{plug}` ✓</blockquote>", parse_mode="html")
    ...
    async def heroku_logs(event):
        ...
        await event.client.send_file(
            ...
            caption="<blockquote>Ultroid Heroku Logs.</blockquote>", parse_mode="html"
        )
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<blockquote><b>Ultroid {ultroid_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b></blockquote>"
        ch_tl = f"```Ultroid {ultroid_version} updates for {ac_br}:```"
        d_form = "%d/%m/%y || %H:%M"
        for c in repo.iter_commits(diff):
            ch_log += f"<blockquote>\n\n💬 <b>{c.count()}</b> 🗓 <b>[{c.committed_datetime.strftime(d_form)}]</b>\n<b><a href={UPSTREAM_REPO_URL.rstrip('/')}/commit/{c}>[{c.summary}]</a></b> 👨‍💻 <code>{c.author}</code></blockquote>"
            tldr_log += f"<blockquote>\n\n💬 {c.count()} 🗓 [{c.committed_datetime.strftime(d_form)}]\n[{c.summary}] 👨‍💻 {c.author}</blockquote>"
        if ch_log:
