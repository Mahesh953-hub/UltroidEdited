    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            if ult.is_private and (groups_only or admins_only):
                return await eod(ult, get_string("py_d3",  parse_mode="html"))
            elif admins_only and not (chat.admin_rights or chat.creator):
                return await eod(ult, get_string("py_d5", parse_mode="html"))
            ...
            try:
                ...
                return await eod(ult, get_string("py_d8", parse_mode="html"))
            except (BotMethodInvalidError, UserIsBotError):
                return await eod(ult, get_string("py_d6", parse_mode="html"))
            except AlreadyInConversationError:
                return await eod(
                    ult,
                    get_string("py_d7", parse_mode="html"),
                )
            except (BotInlineDisabledError, DependencyMissingError) as er:
                return await eod(ult, f"```ERROR:\n{er}```")
            except (
                MessageIdInvalidError,
                MessageNotModifiedError,
                MessageDeleteForbiddenError,
                ...
                ftext = ("<blockquote><i><b>Ultroid Client Error: Forward this to @UltroidSupportChat\n\n<blockquote></i></b>")
                ftext += (f'<blockquote><i><b>Py-Ultroid Version: {pyver}</blockquote></i></b>')
                ftext += (f"<blockquote>\nUltroid Version: {ult_ver}</blockquote></i></b>")
                ftext += (f"<blockquote><i><b>\nTelethon Version: {telever} </blockquote></i></b>")
                ftext += (f"<blockquote><i><b>\nHosted At: {HOSTED_ON}\n\n</blockquote></i></b>")
                ftext += "<blockquote><i><b> -------- ULTROID CRASH LOG -------- </blockquote></i></b>"
                ftext += (f"<blockquote><i><b>\nDate: {date}</blockquote></i></b>")
                ftext += (f"<blockquote><i><b>\nGroup: {ult.chat_id} {naam}</blockquote></i></b>")
                ftext += (f"<blockquote><i><b>\nSender ID: {ult.sender_id}</blockquote></i></b>")
                ftext += (f"<blockquote><i><b>\nReplied: {ult.is_reply}</blockquote></i></b>")
                ftext += "<blockquote><i><b>\n\nEvent Trigger:\n</blockquote></i></b>"
                ftext += (f"```{ult.text}```")
                ftext += "<blockquote><i><b>\n\nTraceback info:\n</blockquote></i></b>"
                ftext += (f"<pre>{format_exc()}</pre>")
                ftext += "<blockquote><i><b>\n\nError text:\n</blockquote></i></b>"
                ftext += (f"<blockquote>{sys.exc_info()[1]}</blockquote>")
                ftext += "<blockquote>\n\n--------END ULTROID CRASH LOG-------- </blockquote>"
                ftext += "<blockquote><i><b>\n\n\nLast 5 commits\n</blockquote></i></b>"

                stdout, stderr = await bash('git log --pretty=format:"%an: %s" -5')
                result = stdout + (stderr or "")

                ftext += f"<pre>{result}</pre>"
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="<pre><code>Ultroid Client Error: Forward this to @UltroidSupportChat\n\n</pre></code>",
                            parse_mode="html"
                        )
                else:
                ...
                if ult.out:
                    await ult.edit(
                        f"<b><blockquote><a href={error_log.message_link}>[OHH! FUCK]</a></blockquote></b>",
                        link_preview=False,
                        parse_mode="html",
                    )
            ...
            async def manager_cmd(ult):
                ...
                try:
                    ...
                    if chat := udB.get_key("MANAGER_LOG"):
                        text = f"<blockquote><i><b>MANAGER_LOG\n\nChat: {get_display_name(ult.chat)} {ult.chat_id}</blockquote></i></b>"
                        text += f"<pre><code>\nReplied : {ult.is_reply} \n Command : {ult.text}\n\n Error :  {er}</pre></code>"
                        try:
                            return await asst.send_message(
                                chat, text, link_preview=False, parse_mode="html"
                            )
                        except Exception as er:
                            LOGS.exception(er)
