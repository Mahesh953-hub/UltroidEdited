    ...
    def ultr(func):
        async def wrapper(event):
            ...
            if from_users and event.sender_id not in from_users:
                return await event.answer("Not For  You Bisshhh!", alert=True)
            if owner and event.sender_id not in owner_and_sudos():
                return await event.answer(f"Demploy Your Own Bishhhh!!")
            try:
                await func(event)
            except Exception as er:
                LOGS.exception(er)
    ...
    def don(func):
        async def wrapper(event):
            if owner and event.sender_id not in owner_and_sudos():
                res = [
                    await event.builder.article(
                        title="Ulʟᴛʀᴏɪᴅ Usᴇʀʙᴏᴛ",
                        url="https://t.me/RemainsAlways",
                        description="©️ @RemainsAlways",
                        text=MSG,
            ...
            try:
                ...
                def error_text():
                    return f"<blockquote> ⚠️ #ERROR #INLINE\n\nQuery: {asst.me.username} {pattern}\n\nTraceback:\n {format_exc()}</blockquote>"
