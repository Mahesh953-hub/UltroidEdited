...
async def autopilot():
    ...
    if not channel:
        ...
        try:
            r = await ultroid_bot(
                CreateChannelRequest(
                    title="Uʟᴛʀᴏɪᴅ Lᴏɢs",
                    about="Jᴜsᴛ ᴀɴ Usᴇʀʙᴏᴛ Lᴏɢs Gʀᴏᴜᴘ!",
                    megagroup=True,
                ),
            )
    ...
    try:
        ...
        try:
            ...
            LOGS.info("```⚠️Error while Adding Assistant to Log Channel```")
            LOGS.exception(er)
            assistant = False
    except BaseException as er:
        assistant = False
    ...
    if assistant and new_channel:
        try:
            ...
            achat = None
            LOGS.info("```⚠️ Error while getting Log channel from Assistant```")
            LOGS.exception(er)
...
async def customize():
    ...
    try:
        ...
        await ultroid_bot.send_message(
            "botfather", f"Pᴏᴡᴇʀғᴜʟʟ Usᴇʀʙᴏᴛ Oғ {sir}"
        )
        await asyncio.sleep(2)
        ...
        await ultroid_bot.send_message(
            "botfather",
            f"Aɴ PᴏᴡᴇʀFᴜʟʟ Usᴇʀʙᴏᴛ Assɪsᴛᴀɴᴛ ✨\n✨ Mᴀsᴛᴇʀ ~ {sir} ✨",
        )
        await asyncio.sleep(2)
...
async def ready():
    ...
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """<blockquote><b>🎇 Thanks for Deploying Ultroid Userbot!
• Here, are the Some Basic stuff from, where you can Know, about its Usage.</b></blockquote>"""
        PHOTO = "https://graph.org/file/54a917cc9dbb94733ea5f.jpg"
        BTTS = Button.inline("• Click to Start •", "initft_2")
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = (f"<blockquote><b>Ultroid has been deployed!</b>\n➖➖➖➖➖➖➖➖➖➖\n<b>UserMode</b>: {inline_mention(ultroid_bot.me)}\n<b>Assistant</b>: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖\n<b>Support</b>: @RemainAlways\n➖➖➖➖➖➖➖➖➖➖</blockquote>")
