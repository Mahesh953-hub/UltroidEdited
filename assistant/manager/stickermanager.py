...
async def kang_cmd(ult):
    ...
    if not get_.get(ult.sender_id) or not get_.get(ult.sender_id, {}).get(type_):
        ...
        title = f" 👾 {get_display_name(sender)}'s Pamck"
        if animated:
            type_ = "anim"
            sn += "_anim"
            title += " (Animated)"
        ...
        return await ult.reply(
            f"<blockquote><b>Kanged Successfully!</b>\n<b>Emoji :</b> {emoji}\n<b>Link :</b> <b>[Click Here](https://t.me/addstickers/{sn})</b></blockquote>", parse_mode="html"
        )
    ...
    try:
        ...
        udB.set_key("STICKERS", get_)
        return await ult.reply(
            f"<blockquote><b>Created New Kang Pack!</b>\n<b>Emoji : </b>{emoji}\n<b>Link : [Click Here](https://t.me/addstickers/{sn})</b></blockquote>", parse_mode="html"
        )
    except Exception as er:
        LOGS.exception(er)
        return await ult.reply(f"```ERROR:\n{er}``m")
    await ult.reply(
        f"<blockquote>Sticker Added to Pack Successfully\nLink : [Click Here](https://t.me/addstickers/{name})</blockquote>"
    )
...
async def do_magic(ult):
    ...
    for _ in ul.keys():
        al_.extend(ul[_])
    msg = "```• Stimkers Owned by You!\n\n```"
    for _ in al_:
        try:
            pack = await ult.client(GetSticker(InputStickerSetShortName(_), hash=0))
            msg += f"<blockquote>• [{pack.set.title}](https://t.me/addstickers/{_})\n</blockquote>"
        except StickerSetInvalidError:
    ...
    await ult.reply(msg, parse_mode="html")
